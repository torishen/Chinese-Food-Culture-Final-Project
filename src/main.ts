...
const ring = document.getElementById('cur-ring') as HTMLElement;
...
function showModal(): void {
  const modal = document.getElementById('audio-modal') as HTMLElement;
  if (modal.dataset.shown === '1' || modal.dataset.dismissed === '1') return;
...
function modalPlay(): void {
  const aud = document.getElementById('main-audio') as HTMLAudioElement;
  const status = document.getElementById('modal-status') as HTMLElement;
...
function dismissModal(): void {
  ...
  setTimeout(() => { if (!modal.classList.contains('show')) modal.style.display = 'none'; }, 600);
}

function _modalScrollCheck(): void {
...
setTimeout(() => {
  const modal = document.getElementById('audio-modal') as HTMLElement;
  if (modal.dataset.shown !== '1') showModal();
}, 4000);
...
const timeEl = document.getElementById('audio-time') as HTMLElement;
let playing = false;
...
function setPlaying(state: boolean): void {
  playing = state;
  (document.getElementById('bar-icon') as HTMLElement).innerHTML = state ? PAUSE : PLAY;
  (document.getElementById('audio-bar') as HTMLElement).classList.toggle('playing', state);
  (document.getElementById('audio-modal') as HTMLElement).classList.toggle('playing', state);
}

function togglePlay(): void {
  const hasSource = audio.querySelector('source') || audio.src;
  if (!hasSource) {
    (document.getElementById('audio-modal') as HTMLElement).style.display = 'flex';
    requestAnimationFrame(() => requestAnimationFrame(() => {
      (document.getElementById('audio-modal') as HTMLElement).classList.add('show');
    }));
    return;
  }
  if (playing) {
...
function showAudioError(msg: string): void {
  let toast = document.getElementById('audio-error-toast') as HTMLElement;
  if (!toast) {
...
const navEl = document.getElementById('chapter-nav') as HTMLElement;
sections.forEach((sec, i) => {
  const pip = document.createElement('div'); pip.className = 'cpip';
  pip.style.top = (8 + i * (82 / (sections.length - 1))) + '%';
  const lbl = document.createElement('span'); lbl.className = 'cpip-lbl'; lbl.textContent = sec.getAttribute('data-chapter');
  pip.appendChild(lbl);
  pip.addEventListener('click', () => sec.scrollIntoView({ behavior: 'smooth' }));
  navEl.appendChild(pip);
});
const pips = navEl.querySelectorAll<HTMLElement>('.cpip');
const chInd = document.getElementById('chapter-ind') as HTMLElement;
...
const barsObs = new IntersectionObserver(entries => {
  entries.forEach(e => {
    if (e.isIntersecting) {
      document.querySelectorAll<HTMLElement>('#bars-wrap .bar-row').forEach((r, i) => {
        const f = r.querySelector<HTMLElement>('.bar-fill');
        const pct = r.getAttribute('data-pct');
        if (f) setTimeout(() => { f.style.width = pct + '%'; }, i * 100);
      });
...
function openDrawer(id: string): void {
  const d = PORTRAITS[id]; if (!d) return;
  let galleryHtml = '';
  ...
  let recipeHtml = '';
  if (d.recipe) { recipeHtml = `<div class="recipe-list">`; d.recipe.forEach(s => recipeHtml += `<div class="recipe-step">${s}</div>`); recipeHtml += `</div>`; }
  let tagsHtml = '';
  if (d.tags) { tagsHtml = `<div class="dr-tags">`; d.tags.forEach(t => tagsHtml += `<span class="dr-tag">${t}</span>`); tagsHtml += `</div>`; }

  (document.getElementById('drawer-body') as HTMLElement).innerHTML = `
    ...
    ${tagsHtml}
  `;
  (document.getElementById('drawer') as HTMLElement).classList.add('open');
}

function closeDrawer(): void { (document.getElementById('drawer') as HTMLElement).classList.remove('open'); }
document.addEventListener('click', e => {
  const dr = document.getElementById('drawer') as HTMLElement;
  const target = e.target as Element;
  if (dr.classList.contains('open') && !dr.contains(target) && !target.closest('.portrait-card')) closeDrawer();
});
...
function openLightboxSrc(src: string, caption: string): void {
  const lb = document.getElementById('lightbox') as HTMLElement;
  (document.getElementById('lightbox-img') as HTMLImageElement).src = src;
  (document.getElementById('lightbox-caption') as HTMLElement).textContent = caption || '';
  lb.classList.add('show');
}
function closeLightbox(): void { (document.getElementById('lightbox') as HTMLElement).classList.remove('show'); }
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') { closeDrawer(); closeLightbox(); dismissModal(); }
});
...
const fullAudio = (() => {
  ...
  if (src) a.src = src.getAttribute('src') ?? '';
  return a;
})();
...
function toggleFull(): void {
  const btn = document.getElementById('full-play-btn') as HTMLElement;
  const icon = document.getElementById('full-icon') as HTMLElement;
  const label = document.getElementById('full-play-label') as HTMLElement;
  const waveform = document.getElementById('full-waveform') as HTMLElement;
...
fullAudio.addEventListener('timeupdate', () => {
  ...
  const fill = document.getElementById('full-prog-fill') as HTMLElement;
  const fullTimeEl = document.getElementById('full-time') as HTMLElement;
  if (fill) fill.style.width = (fullAudio.currentTime / fullAudio.duration * 100) + '%';
  if (fullTimeEl) {
...
fullAudio.addEventListener('ended', () => {
  ...
  const btn = document.getElementById('full-play-btn') as HTMLElement;
  const waveform = document.getElementById('full-waveform') as HTMLElement;
  if (icon) icon.innerHTML = FULL_PLAY;
...
function fullSeek(e: MouseEvent): void {
  if (!fullAudio.duration) return;
  const r = (e.currentTarget as HTMLElement).getBoundingClientRect();
...
(function () {
  ...
  grid.addEventListener('touchstart', (e: TouchEvent) => { tStart = e.touches[0].pageX; tScroll = grid.scrollLeft; }, { passive: true });
  grid.addEventListener('touchmove', (e: TouchEvent) => {
    grid.scrollLeft = tScroll - (e.touches[0].pageX - tStart);
  }, { passive: true });
})();

function galleryScroll(dir: number): void {
  const grid = document.getElementById('gallery-grid') as HTMLElement;
  if (!grid) return;
